...
func (p *actorPlayer) playerEnter(session *cproto.Session, req *pb.Int64) {
	...
	// 这里改为客户端主动请求更佳
	// [01]推送角色 道具数据
	//online.Item.ListPush(session, playerId)
	// [02]推送角色 英雄数据
	//online.Hero.ListPush(session, playerId)
	// [03]推送角色 成就数据
	//online.Achieve.CheckNewAndPush(playerId, true, true)
	// [04]推送角色 邮件数据
	//online.Mail.ListPush(session, playerId)
