...
func (r *PlayerRecord) String() string {
	return fmt.Sprintf("PlayerRecord{playerid:%v}", r.PlayerId)
}
...
var _PlayerSource = &xdb.Source{
	...
	PKOf: func(obj interface{}) xdb.PK {
		// 支持 PlayerRecord 和 Player 两种类型
		var playerId int64
		if r, ok := obj.(*PlayerRecord); ok {
			playerId = r.PlayerId
		} else if p, ok := obj.(*Player); ok {
			playerId = p.PlayerId
		} else {
			panic(fmt.Sprintf("invalid type for PKOf: %T", obj))
		}
		return &PlayerPK{
			PlayerId: playerId,
		}
...
func init() {
	_PlayerSource.PKOf = func(obj interface{}) xdb.PK {
		// 支持 PlayerRecord、Player、PlayerCommitment 和 PlayerModel 类型
		var playerId int64
		if r, ok := obj.(*PlayerRecord); ok {
			playerId = r.PlayerId
		} else if p, ok := obj.(*Player); ok {
			playerId = p.PlayerId
		} else if c, ok := obj.(*PlayerCommitment); ok {
			// 从 Commitment 中获取 PlayerId
			if c.data != nil {
				playerId = c.data.PlayerId
			}
		} else {
			// 尝试通过反射获取 PlayerId（支持 PlayerModel 等嵌入 PlayerRecord 的类型）
			...
			if val.Kind() == reflect.Struct {
				// 查找 PlayerId 字段
				playerIdField := val.FieldByName("PlayerId")
				if playerIdField.IsValid() && playerIdField.CanInterface() {
					if id, ok := playerIdField.Interface().(int64); ok {
						playerId = id
					} else {
						panic(fmt.Sprintf("invalid PlayerId type for PKOf: %T", obj))
					}
				} else {
					// 尝试查找嵌入的 PlayerRecord
					for i := 0; i < val.NumField(); i++ {
						field := val.Field(i)
						if field.Kind() == reflect.Struct {
							playerIdField := field.FieldByName("PlayerId")
							if playerIdField.IsValid() && playerIdField.CanInterface() {
								if id, ok := playerIdField.Interface().(int64); ok {
									playerId = id
									break
...
func (r *ItemRecord) String() string {
	return fmt.Sprintf("ItemRecord{playerid:%v}", r.PlayerId)
}
