...
- 找到`server/nodes/main.go`，所有节点都从`main.go`启动
...
- `GoLand`用户配置启动参数(`Program arguments`)如下：
  - `gc-master`       master --path=./config/server.json --node=gc-master
  - `gc-center`       center --path=./config/server.json --node=gc-center
  - `gc-web-1`        web --path=./config/server.json --node=gc-web-1
  - `gc-gate-1`       gate --path=./config/server.json --node=gc-gate-1
  - `gc-game-10001`   game --path=./config/server.json --node=10001
...
- 找到`server/robot_client/main.go` 文件,并执行
